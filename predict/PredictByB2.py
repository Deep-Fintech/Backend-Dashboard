...
predict_by_B2_API = Blueprint('predict_by_B2', __name__)


# @predict_by_B2_API.route('/predict_by_B2')
def predict_by_B2():
    req = requests.get(
    ...
    for i in range(2):
        X.append(scaled_kline_data[i:i+50])

    X = np.array(X)

    model = load_model("model/model_b2.h5")
